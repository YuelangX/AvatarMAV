...
class HeadNeRFInferRecorder():
    ...
    def log(self, log_data):
        ...
        data = {
            'image': log_data['data']['image'][0:1],
            'exp': log_data['data']['exp'][0:1],
            'pose': log_data['data']['pose'][0:1],
            'scale': log_data['data']['scale'][0:1],
            'intrinsic': log_data['data']['intrinsic'],
            'extrinsic': log_data['data']['extrinsic'],
        }
        with torch.no_grad():
            data = log_data['camera'].render(data, data['image'].shape[2])
        render_image = data['render_image']
