...
class HeadModule(nn.Module):
    def __init__(self, opt):
        ...
        self.deform_bs_volume = nn.Parameter(torch.zeros([opt.exp_dim, opt.deform_bs_dim, opt.deform_bs_res + 1, opt.deform_bs_res + 1, opt.deform_bs_res + 1]))
        self.deform_mean_volume = nn.Parameter(torch.zeros([opt.deform_bs_dim, opt.deform_bs_res + 1, opt.deform_bs_res + 1, opt.deform_bs_res + 1]))
        self.deform_linear = MLP(opt.deform_linear)

        self.feature_volume = nn.Parameter(torch.zeros([opt.feature_dim, opt.feature_res + 1, opt.feature_res + 1, opt.feature_res + 1]))
